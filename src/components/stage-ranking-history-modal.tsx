...
export function StageRankingHistoryModal({
  canEditStageRanking = false,
  onClose,
  snapshot,
  stageId,
}: {
  ...
  const selectedStageMatches = useMemo(
    ...
    [snapshot.stageMatchPoints, stageId],
  );
  const selectedStageHistoryDetail = useMemo(
    () => snapshot.stageHistoryDetails.find((stage) => stage.stageId === stageId) ?? null,
    [snapshot.stageHistoryDetails, stageId],
  );
  ...
  if (!selectedStageMatches || !selectedStageHistoryDetail) {
    return null;
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-6xl overflow-hidden rounded-[1.55rem] border border-[rgba(255,208,101,0.16)] bg-[linear-gradient(180deg,rgba(12,44,31,0.98),rgba(7,24,18,0.99))] shadow-[0_28px_60px_rgba(0,0,0,0.42)]">
        ...
        <div className="max-h-[calc(90vh-96px)] overflow-y-auto">
          <div className="border-b border-[rgba(255,208,101,0.1)] px-5 py-5 md:px-6">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <HistoryInfoCard label="Inicio programado" value={selectedStageHistoryDetail.scheduledStartLabel} />
              <HistoryInfoCard label="Inicio real" value={selectedStageHistoryDetail.actualStartLabel} />
              <HistoryInfoCard label="Fim real" value={selectedStageHistoryDetail.actualEndLabel} />
              <HistoryInfoCard
                label="Duracao total"
                value={formatDuration(selectedStageHistoryDetail.totalDurationSeconds)}
              />
              <HistoryInfoCard label="Premio do dia" value={selectedStageHistoryDetail.dailyPrize} />
              <HistoryInfoCard
                label={selectedStageHistoryDetail.isTest ? "Pote anual" : "Pote anual gerado"}
                value={
                  selectedStageHistoryDetail.isTest
                    ? "Nao contabilizado"
                    : selectedStageHistoryDetail.annualPotContribution
                }
              />
              <HistoryInfoCard
                label="Partidas jogadas"
                value={String(selectedStageHistoryDetail.matchesPlayed)}
              />
              <HistoryInfoCard
          ...
          <div className="grid gap-5 border-t border-[rgba(255,208,101,0.1)] px-5 py-5 md:px-6 md:py-6 xl:grid-cols-[1.05fr_0.95fr]">
            <div className="rounded-[1.35rem] border border-[rgba(255,208,101,0.12)] bg-[rgba(255,255,255,0.03)] p-4 md:p-5">
              <h4 className="text-2xl font-semibold text-[rgba(255,220,143,0.98)]">
              ...
              <div className="mt-4 overflow-x-auto rounded-[1.1rem] border border-[rgba(255,208,101,0.1)]">
                <table className="min-w-full border-collapse">
                  ...
                  <tbody>
                    {selectedStageHistoryDetail.finalRanking.map((entry, index) => (
                      <tr
                        key={entry.playerId}
                        className={
                          index % 2 === 0
            ...
            <div className="rounded-[1.35rem] border border-[rgba(255,208,101,0.12)] bg-[rgba(255,255,255,0.03)] p-4 md:p-5">
              ...
              <div className="mt-4 grid gap-3">
                {selectedStageHistoryDetail.matches.map((match) => (
                  <div
                    key={match.matchNumber}
